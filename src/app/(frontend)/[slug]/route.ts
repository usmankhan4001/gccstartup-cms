...
import { countryHTML, htmlResponse } from '@/lib/renderSite'
...
export async function GET(request: Request, { params }: { params: Promise<{ slug: string }> }) {
  try {
    ...
    const [res, settings] = await Promise.all([
      payload.find({ collection: 'countries', where: { slug: { equals: slug } }, limit: 1 }),
    ...
    return htmlResponse(countryHTML(doc, locale, settings))
  } catch (err: any) {
    console.error('[slug GET] Server-side crash caught:', err)
    return htmlResponse(`
