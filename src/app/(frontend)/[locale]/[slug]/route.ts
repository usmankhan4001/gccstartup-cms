...
import { countryHTML, htmlResponse, isLocale } from '@/lib/renderSite'

export const dynamic = 'force-dynamic'

// /<locale>/<country-slug>  (e.g. /de/uae, /ar/bahrain)
export async function GET(_req: Request, { params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  const res = await payload.find({
    collection: 'countries', where: { slug: { equals: slug } }, limit: 1, locale: locale as any,
  })
