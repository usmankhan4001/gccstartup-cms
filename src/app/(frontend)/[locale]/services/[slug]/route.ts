...
import { serviceHTML, htmlResponse, isLocale } from '@/lib/renderSite'
...
export async function GET(_req: Request, { params }: { params: Promise<{ locale: string; slug: string }> }) {
  ...
  const res = await payload.find({
    collection: 'services', where: { slug: { equals: slug } }, limit: 1, locale: locale as any,
  })
  const doc = res.docs[0]
  if (!doc) return htmlResponse('<h1>Page not found</h1><p><a href="/">Back home</a></p>', 404)
  return htmlResponse(serviceHTML(doc, locale))
}
